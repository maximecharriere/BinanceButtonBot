...
def main():
    try:
        ...
        try:
            ...
            if REMOTE_WEBDRIVER:
                driver = webdriver.Remote(
                    "http://localhost:4444/wd/hub", options=chrome_options)
            else:
        ...
        while True:
            ...
            countdown.append(
                float(matches[0].text+matches[1].text+'.'+matches[2].text+matches[3].text))
            if (start_s >= start_m+60.0):
                ...
                try:
                    mycursor.execute(
                        MYSQL_QUERY, (start_m, participants, countdown_min))
                    mydb.commit()
                except mysql.connector.Error as e:
                    print(f"{type(e).__name__} : {e}")
                    return 13
